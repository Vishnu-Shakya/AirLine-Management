...
const Modal = ({ show, onClose, flightOffer, flight, SERVER_URL }) => {
    ...
    const handleContinuePay = async (e) => {
        ...
        try {
            const response = await axios.post(SERVER_URL + "/payment", formData);
            const { id: order_id, amount, currency } = response.data;
            // Set up RazorPay options
            const options = {
                key: "rzp_test_a7jt1IP0RVwSiE", // Replace with your RazorPay Key ID
                amount: amount,
                currency: currency,
                name: "SkyTrip",
                ...
                handler: (response) => {
                    ...
                    toast.success(`Payment successful! ID: ${paymentId}. Your ticket is being processed.Please save this ID.`);
                },
                prefill: {
                    name: "John Doe",
    ...
    const totalFare = adults * basePrice + children * basePrice * 0.1 + (totalPrice - basePrice) * (adults + children);
                ...
                <div className="modal-body">
                        ...
                        <p>
                            {segments.map((segment, index) => {
                                ...
                                return (
                                    <span key={index}>
                                        {index > 0 && " -> "}
                                        {airport.city} ({iata}){index === segments.length - 1 && ` -> ${airport1.city} (${iata1})`}
                                    </span>
                                );
                            })}
                        </p>
                    ...
                    <div className="modal-main-info">
                        {segments.map((segment, index) => {
                            ...
                            const layoverTime = index >= 0 ? calculateLayoverTime(segment.departure.at, segment.arrival.at) : null;

                            return (
                                <div
                                    key={index}
                                    className="modal-segment-info"
                                >
                                    ...
                                    <p>
                                        <strong>Departure Time:</strong> {format(new Date(segment.departure.at), "yyyy-MM-dd HH:mm")}
                                    </p>
                                    <p>
                                        <strong>Arrival Time:</strong> {format(new Date(segment.arrival.at), "yyyy-MM-dd HH:mm")}
                                    </p>
                                    {/* <p>
                    ...
                    <div className="modal-additional-info">
                        ...
                        <form className="modal-passenger-form">
                            ...
                            {Array.from({ length: adults + children }).map((_, i) => (
                                <div
                                    key={i}
                                    className="modal-passenger-info"
                                >
                                    <h4>Passenger {i + 1}</h4>
                                    <div className="modal-form-group">
                                        <input
                            ...
                            <p>
                                <strong>Total Fare:</strong> {totalFare.toFixed(2)} {flightOffer.flightOffers[0].price.currency}
                            </p>
                            <button
